...
export default function CartBox() {
    ...
    return (
        <Link href={'/panel/cart'} className={`${styles.shoppingCart} cursor-pointer border`}>
            {auth.user && auth.user.cart_products_total_number > 0 && <div className={`bg-green-light ${styles.badgee}`}>{auth.user.cart_products_total_number}</div>}
