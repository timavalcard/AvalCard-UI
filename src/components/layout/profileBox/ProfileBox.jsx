...
export default function ProfileBox() {
    ...
    return (
        <div className={`flex justify-center items-center cursor-pointer relative ml-4 ${styles.userBox} ${styles.profileParent}`}>
            <div>
                <img width={33} height={33} src={auth.user?.avatar} alt={auth.user?.name} />
            </div>
            <div className='flex items-baseline'>
                <div className={`grid justify-start mx-2`}>
                    <Link href={'/profile'} className='text-very-bold'>{auth.user?.name}{auth.user?.last_name ? " " + auth.user.last_name : ""}</Link>
                   {/* <p className={`${styles.subTitle}`}>متن امتحانی برای زیرتایتل</p>*/}
                </div>
                <div>
                    <svg width="13" className='mt-0.5' viewBox="0 0 13 7" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className={`custom-shadow text-xs font-medium ${styles.profilePanel}`}>
                <Link href={'/profile'} className={styles.header}>
                    <div className="flex items-start gap-1">
                        <div>
                            <img width={20} height={20} src={auth.user?.avatar} alt={auth.user?.name} className='-mt-[2px]' />
                        </div>
                        <div>
