...
import {addToCart} from "../../../../helpers/api/cart/addToCart"
...
export default  function Cart(params) {
    ...
    const handleAddToCart =async () => {
        setLoading(true)
        if(params.variationId || !params.isVariable){
            await addToCart( params.productId,params.variationId,params.quantity)
    ...
    return (
        <div className="md:col-span-4 h-min">
        <div className=" border shadow-lg border-[#F0F0F0] text-[#A9A9A9] rounded-2xl py-8 px-5 text-xs font-bold grid gap-3">
            {
                params.discount && params.discountPercentage
                &&
                ...
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center font-medium gap-2">
                        ...
                        <DiscountPercentage per={params.discountPercentage} />
                    </div>
                </div>
            }
            <div className="flex justify-between items-center">
                ...
                <div className="text-[#202020] ">
                    {params.totalAmount !== "لطفا انتخاب کنید" ?

                        params.totalAmount.toLocaleString("fa") + " تومان "
                        :
                        params.totalAmount
                    }
            ...
            {/*{params.feeAmount > 0 && <div className="flex justify-between items-center">
                <div>
                    مبلغ کارمزد:
                </div>
                <div className="text-[#202020] ">
                    {params.feeAmount.toLocaleString() + " تومان "}
                </div>
            </div> }
            {params.tenPercentValue > 0 && <div className="flex justify-between items-center">
                <div>
                    مالیات و ارزش افزوده:
                </div>
                <div className="text-[#202020] -2xl">
                    {params.tenPercentValue.toLocaleString() + " تومان "}
                </div>
            </div> }*/}

            {params.finalRialAmount > 0 && <div className="flex justify-between items-center">
                <div>
                    مبلغ نهایی پرداختی به تومان:
                </div>
                <div className="text-[#202020] ">
                    {params.finalRialAmount.toLocaleString() + " تومان "}
                </div>
            </div> }

            <div onClick={handleAddToCart}>
                <Button flex size="full"
                        gradient={!params.isVariable || (params.isVariable && params.variationId) ? 'blue' : 'gray'}
                        loading={loading}
                        className={'!rounded-xl !h-[3.3rem] mt-2'}>
                    <div>
        ...
        <div className="mt-5">
            {!params.currency_key && <PriceDollar dollar={params.dollar_price} /> }
            {params.currency_key && <PriceDollar currency={params.currency_key} dollar={`${params.exchange_rate?.toLocaleString("fa-IR")}  تومان `} /> }
        </div>
        </div>
    );
}