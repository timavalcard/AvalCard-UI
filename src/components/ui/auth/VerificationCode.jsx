...
import {loginByCode, register as registerAction} from "../../../helpers/actions/authActions";
import Button from "../globals/Button";
import {sendLoginCode} from "../../../helpers/api/sendLoginCode";
...
export default function VerificationCode({ email }) {
    ...
    const onSubmit = (data) => {
        if (!validate()) {
            setLoading(true)
            dispatch(loginByCode(email, code.join(''), () => {
                router.push('/panel');
            }));
            setLoading(false)
        }
                ...
                <div className="lg:col-span-5 grid">
                    <div className={`${styles.boxLoginSignup}`}>
                        <div className={`${styles.loginSignup}`}>
                            ...
                            <div className='mt-8'>
                                <p className='text-very-bold text-center lg:text-right'>کد یک بار مصرف را وارد کنید:</p>
                            </div>
                            <div className='mt-4'>
