...
export default function ClientContent({ categories,baseUrl=false }) {
    ...
    const categoryTabs = [
        { title: 'همه گیفت‌کارت‌ها', icon: null },
        ...
        <>
            ...
            {active === 0 ? (
                categories.map((category, index) => (
                    <div key={index} className="FadeInAnimate mt-custom-4 section" id={`gift-card-box-${category.id}`}>
                ...
                <div className="FadeInAnimate mt-custom-4 section" id="gift-card-box">
                    ...
                    <div className='grid lg:grid-cols-4 md:grid-cols-3 grid-cols-2 items-center justify-center xl:gap-10 gap-5'>
                        {categories[active - 1].products.length > 0 && categories[active - 1]?.products.map((product) => (
                            <>
                                {product.url && <Card
                                    key={product.id}
                                    alt={product.media?.alt}
