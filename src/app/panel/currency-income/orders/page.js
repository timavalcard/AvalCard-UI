...
import {getCurrencyIncomeOrders} from "../../../../helpers/api/currencyIncome/getOrders";
...
export default async function Page() {
  ...
  const orders = await getCurrencyIncomeOrders()
  return (
            ...
            {orders && orders.length && orders.map((order, index) => (
              <tr key={order.id} className="">
                ...
                <td className="!text-xs !text-black">
                  <OrderType order_type={"currency_income"} />
                </td>
                ...
                <td>
                  <Link href={`/panel/currency-income/orders/${order.id}`} className="flex items-center align-middle gap-1 cursor-pointer text-center justify-center transition-all !text-[#909090] hover:!text-[#3b3b3b]">
